...
│                                                                     │
│  ┌──────────┐    ┌──────────────┐    ┌──────────────┐              │
│  │  用户     │───>│ YouTube URL  │───>│ YouTube API  │              │
│  │ (Nike    │    │ 输入          │    │ commentThreads│             │
│  │  营销人员)│    └──────────────┘    └──────┬───────┘              │
│  └──────────┘                                │                      │
│                                              v                      │
│                                   ┌──────────────────┐             │
...
│  │ 7 类            │────────────>│ uncased              │                │
│  │ (17,166 训练集) │   阶段 1    │ (基础模型)           │                │
│  └─────────────────┘   微调      └──────────┬───────────┘                │
│                                              │                           │
│                                              v                           │
│                                   ┌──────────────────────┐                │
│  ┌─────────────────┐              │ GoEmotions           │                │
│  │ YouTube 域      │              │ 微调 DistilBERT      │                │
│  │ DeepSeek 标注   │────────────>│ chase1zhang/         │                │
│  │ (3,991 训练集)  │   阶段 2    │ youtube-emotion-     │                │
│  └─────────────────┘   域适配    │ distilbert           │                │
│                                  └──────────┬───────────┘                │
│                                              │                           │
│                                              v                           │
...
│                                   └──────────────────────┘                │
│                                                                          │
│  额外对比模型（在 Colab 中同样进行了域适配）：                            │
│  ┌────────────────────────┐  ┌────────────────────────┐                  │
│  │ SamLowe RoBERTa-base   │  │ j-hartmann Distil-     │                  │
│  │ (公开 GoEmotions)      │  │ RoBERTa-base (公开)    │                  │
│  │ → 域适配               │  │ → 域适配               │                  │
│  │   验证 0.6504          │  │   验证 0.6291          │                  │
│  └────────────────────────┘  └────────────────────────┘                  │
│                                                                          │
...
│  │ RoBERTa-base (公开)    │  │ large (公开)           │                  │
│  │ 验证 0.4574            │  │ 验证 0.5138            │                  │
│  └────────────────────────┘  └────────────────────────┘                  │
│                                                                          │
│  七情绪准确率（500 条评论应用基准）：                                      │
│                                                                          │
│  领域适配 DistilBERT       ████████████████████████████  317/500         │
│  领域适配 RoBERTa          ████████████████████████████  315/500         │
│  领域适配 DistilRoBERTa    ██████████████████████████    306/500         │
│  公开 GoEmotions RoBERTa   █████████████████             208/500         │
│  公开 RoBERTa-large        ████████████████              185/500         │
│  微调前基线                 ███████████████               179/500         │
│  GoEmotions 微调           ████████████                  141/500         │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
...
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  以下三个模型均为预训练模型（项目未进行微调）。                            │
│  每条评论被分类为：正面 / 中性 / 负面。                                   │
│                                                                          │
│  ┌────────────────────────────────────────────────────────────────┐      │
│  │  CardiffNLP twitter-roberta-base-sentiment-latest  ★ 推荐      │      │
│  │  任务：三分类情感（正面 / 中性 / 负面）                          │      │
│  │  应用基准：334 / 500 (0.6680)                                  │      │
│  │  优势：最稳定的业务级信号                                        │      │
│  └────────────────────────────────────────────────────────────────┘      │
...
│  │  任务：三分类情感（neg / neu / pos）                            │      │
│  │  应用基准：228 / 500 (0.4560)                                  │      │
│  │  说明：标签格式不同；模型输出正确，但自动准确率脚本低估了匹配数  │      │
│  └────────────────────────────────────────────────────────────────┘      │
│                                                                          │
│  三情感准确率（500 条评论应用基准）：                                      │
│                                                                          │
...
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  结合 Pipeline 1（情绪）+ Pipeline 2（情感）输出。                        │
│  基于规则的启发式方法 → 活动行动 + 风险等级。                             │
│                                                                          │
│  Pipeline 1                    Pipeline 2                                │
│  (七情绪)                      (三情感)                                  │
│       │                             │                                    │
...
CardiffNLP 在 500 条评论应用基准上表现最佳（334/500），提供最稳定的正面/中性/负面信号。lxyuan 推理速度最快，适合延迟敏感场景。

### 11.4 已部署应用性能

**500 条评论应用基准总体性能（10 个视频）：**

| 任务 | 模型 / Pipeline | 匹配 / 500 | 准确率 |
|---|---|---:|---:|
| 七情绪 | 微调前基线 | 179 / 500 | 0.3580 |
| 七情绪 | GoEmotions 微调 | 141 / 500 | 0.2820 |
| 七情绪 | YouTube 域适配 DistilBERT | 317 / 500 | **0.6340** |
| 七情绪 | 公开 GoEmotions RoBERTa | 208 / 500 | 0.4160 |
| 七情绪 | 公开 RoBERTa-large | 185 / 500 | 0.3700 |
| 七情绪 | YouTube 域适配 RoBERTa | 315 / 500 | **0.6300** |
| 七情绪 | YouTube 域适配 DistilRoBERTa | 306 / 500 | 0.6120 |
| 三情感 | CardiffNLP 情感 pipeline | 334 / 500 | **0.6680** |
| 三情感 | lxyuan 情感 | 306 / 500 | 0.6120 |
| 三情感 | BERTweet 情感 | 228 / 500 | 0.4560 |
...
- 在 500 条评论应用基准（10 个视频）上，YouTube 域适配 DistilBERT 在项目自有模型中表现最佳（**317/500, 0.6340**），域适配 RoBERTa 紧随其后（**315/500, 0.6300**）。两者均显著优于非适配基线（微调前 0.3580，GoEmotions 微调 0.2820）。
- 域适配改进最明显的是情绪特定视频：愤怒品牌危机（0.72）、厌恶食品安全（0.72）和惊讶产品失败（0.72）。最难的视频仍是枪击新闻（0.34），讽刺和政治语境对所有模型构成挑战。
- 三分类情感 pipeline 达到 **334/500（0.6680）** 准确率，提供稳定的商业级信号。细粒度的七情绪分类对诊断和模型对比更有用。
...
实验结果表明，微调提高了原始 GoEmotions 基准上的性能，使用平衡 YouTube 数据的域适配在 YouTube 域验证上给出了最强的项目自有模型。公开 SamLowe RoBERTa 模型是最佳应用基准对比模型，而 YouTube 域适配 DistilBERT 仍是最终默认模型，因为它紧凑、为项目自有，且在专用 YouTube 验证集上最强。辅助情感模型在广泛的正面/中性/负面层面表现最佳（334/500），七情绪模型提供了更详细的诊断洞察。两个 pipeline 共同为活动监控和观众反馈分析提供了有用的工作流。
